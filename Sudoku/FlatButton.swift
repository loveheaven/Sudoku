...
@objc open class FlatButton: NSButton, CALayerDelegate {
    ...
    @IBInspectable public var enableHover: Bool = true {
        didSet {
            animateColor(state == .on, isHover);
        }
    }
    @IBInspectable public var enableToggle: Bool = false {
        didSet {
            
        }
    }
    @IBInspectable public var toggleX: CGFloat = 0
    @IBInspectable public var toogleY: CGFloat = 4
    ...
    @IBInspectable public var hoverButtonColor: NSColor = NSColor.white {
        didSet {
            animateColor(state == .on)
        }
    }
    ...
    var toggleButton: DSFToggleButton? = nil;
    override open func awakeFromNib() {
        super.awakeFromNib()
        let trackingArea = NSTrackingArea(rect: bounds, options: [.activeAlways, .inVisibleRect, .mouseEnteredAndExited], owner: self, userInfo: nil)
        addTrackingArea(trackingArea)
        if enableToggle {
            toggleButton = DSFToggleButton(frame:NSMakeRect(self.toggleX, self.toogleY, self.bounds.width/4, self.bounds.height/4));
            toggleButton?.showLabels = true;
            toggleButton?.color = NSColor(red: 51/255.0, green: 157/255.0, blue: 1.0, alpha: 1.0);
            toggleButton?.target = self
            toggleButton?.action = #selector(toggleClick)
            
            self.addSubview(toggleButton!);
        }
    }
    
    @objc public func toggleClick() {
    ...
    public func animateColor(_ isOn: Bool, _ isHover: Bool = false) {
        removeAnimations()
        let duration = isOn ? onAnimationDuration : offAnimationDuration
        let bgColor = isOn ? activeButtonColor : ((enableHover && isHover) ? hoverButtonColor : buttonColor);
